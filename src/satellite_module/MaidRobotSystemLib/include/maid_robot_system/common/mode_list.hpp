 ...
 *
 */
#ifndef MAID_ROBOT_SYSTEM_LIB_COMMON_INTERFACE_MODE_LIST_HPP
#define MAID_ROBOT_SYSTEM_LIB_COMMON_INTERFACE_MODE_LIST_HPP
...
enum ModeList
{
    ...
    MODE_ERROR_NOT_STOP,          /*!< [ERROR] Stop processing could not be executed. */
};
...
inline char *text_contorl_mode(ModeList mode)
{
    ...
    switch (mode) {
        ...
        case ModeList::MODE_ERROR_NOT_COMMUNICATION:
            ...
            break;
        case ModeList::MODE_ERROR_NOT_STOP:
            sprintf(buffer, "<ERROR>CAN'T SYSTEM STOP");
            break;

        default:
            sprintf(buffer, "UNKNOW[%d]", (int)mode);
            break;
    }
    return buffer;
}
