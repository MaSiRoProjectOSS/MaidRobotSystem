 ...
 *
 */
#include "wheel_controller/controller/control_leg_motor.hpp"
...
bool ControlLegMotor::_calculate()
{
    ...
    ///
#if 0
    // TODO : leg_motor.resident()内で毎回有効にしている。
    // CANからの受信データで制御すべきでは？
    // ！！！！！！！！！！！！要確認！！！！！！！！！！！！
    this->enable();
#endif
...
void ControlLegMotor::debug_output()
{
    String message;
    char buffer[255];
    sprintf(buffer, "\t leg_sen: %d", this->_mt->sensor_data);
    message += buffer;
    sprintf(buffer, "\t leg_now:  %d,", this->_mt->now_pos);
    message += buffer;
    sprintf(buffer, "\t leg_tar: %d,", this->_mt->target_pos);
    message += buffer;
    sprintf(buffer, "\t leg_target_speed: %d,", this->_mt->leg_target_speed);
    message += buffer;
    sprintf(buffer, "\t speed: %f", (double)this->_mt->set_speed);
    sprintf(buffer, "\t freq: %f", (double)this->_mt->output_freq);
    message += buffer;
