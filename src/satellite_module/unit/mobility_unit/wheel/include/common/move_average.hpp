 ...
 *
 */
#ifndef COMMON_MOVE_AVERAGE_HPP
#define COMMON_MOVE_AVERAGE_HPP
...
template <int NUM>
int MoveAverage<NUM>::set(int value)
{
    long sum_data = value;
    ...
    this->_data_array[this->_data_size - 1] = value;
    this->average                           = sum_data / this->_data_size;
    return this->average;
}
...
double MoveAverage<NUM>::set_delta(int value)
{
    ...
    for (int i = 0; i < (this->_delta_size - 1); i++) {
        this->_data_array[i]  = this->_data_array[i + 1];
        this->_delta_array[i] = this->_delta_array[i + 1];
        sum_data += this->_data_array[i];
        sum_time += this->_delta_array[i];
    }
    this->_data_array[this->_delta_size - 1] = value;
    sum_data += this->_data_array[this->_delta_size - 1];

    this->_delta_array[this->_delta_size - 1] = elapsed;
    sum_time += this->_delta_array[this->_delta_size - 1];

    this->average       = sum_data / this->_delta_size;
