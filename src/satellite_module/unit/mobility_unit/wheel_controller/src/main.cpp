...
#define APP_NAME    "Wheel controller"
...
WheelController wheel(&SerialEX);
...
void setup(void)
{
    ...
    M5.begin(cfg);
    delay(1000);
    bool result = wheel.begin(MODBUS_ADDRESS, (MessageFrame::MODBUS_TYPE)CUSTOM_MODBUS_TYPE);
    if (false == result) {
...
void update_imu(void)
{
    ...
    if (m5::IMU_Class::sensor_mask_t::sensor_mask_none < imu_update) {
        ...
        wheel.set_accel(data.accel.x, data.accel.y, data.accel.z);
        wheel.set_gyro(data.gyro.x, data.gyro.y, data.gyro.z);
    }
}
...
void loop(void)
{
    ...
    if (0 < count_down) {
        ...
        if (0 == (count_down % 10)) {
            ...
            M5.Lcd.setRotation(2);
            M5.Lcd.printf("%s\r\n version: %s\r\n\r\n", APP_NAME, APP_VERSION);
            M5.Lcd.printf("Accel:\r\n %6d,%6d,%6d\r\n", (int)wheel.accel.x, (int)wheel.accel.y, (int)wheel.accel.z);
            M5.Lcd.printf("Gyro:\r\n %6d,%6d,%6d\r\n", (int)(wheel.gyro.x), (int)(wheel.gyro.y), (int)(wheel.gyro.z));

            M5.Lcd.printf("\r\nCount [%6d/%6d]\r\n", wheel.count_process, wheel.count_reception);
            M5.Lcd.printf("STACK [%6d/%6d]\r\n", (int)(stack_cushy), (int)max_cushy);
            M5.Lcd.printf("OSC-FREQ [%10d]\r\n", wheel.get_oscillator_frequency());
