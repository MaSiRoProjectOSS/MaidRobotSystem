...
#endif
#ifndef WAIST_CONTROLLER_SETTING_FILE_NAME
#define WAIST_CONTROLLER_SETTING_FILE_NAME "/setting.ini"
#endif

class WaistController : public ModbusLibArduino {
public:
    struct vector_t {
        union {
            int16_t value[3];
...
public:
    WaistController(HardwareSerial *serial, int pwm_size = 3, int pwm_address = PCA9685_I2C_ADDRESS);
    ~WaistController();
...
private:
    const char *_setting_file_name = WAIST_CONTROLLER_SETTING_FILE_NAME;
};
